...
| `timetable/` | Personal timetable: terms, week view, agenda, whole-term overview for the poster, stored entries with per-week overrides (scoped edits) and catalog links, catalog search and quick add (旁听), portal/text import, settings (source toggles, hidden tags), ICS link, subscribe-message quota, exam schedule source, poster share assets. Contract in `timetable/README.md` §4, §6, §8 and §10. | `/api/v2/timetable/` |
