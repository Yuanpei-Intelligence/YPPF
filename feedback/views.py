...
def viewFeedback(request: HttpRequest, fid):
        ...
        if public == "public":
            ...
            if request.user.is_org() and feedback.org == me:
                ...
                with transaction.atomic():
                    ...
                    feedback.save()
                    succeed_message.append(
                        "成功修改组织公开状态为【公开】！通过学院审核后，该反馈将向所有人公开。")
                    inform_notification(me, feedback.person,
            ...
            # 发布者（个人）选择公开反馈
                ...
                with transaction.atomic():
                    ...
                    feedback.save()
                    succeed_message.append(
                        "成功修改个人公开状态为【公开】！待小组公开并通过学院审核后，该反馈将向所有人公开。")
            ...
            # 教师选择公开反馈
                ...
                with transaction.atomic():
                    ...
                    inform_notification(
                        me, feedback.org, f"已公开您处理的反馈[{feedback.title}]。", feedback, anonymous=False)
            # 其他人没有公开反馈权限
            else:
        ...
        if public == "private":
            ...
            # 教师选择隐藏反馈
                ...
                with transaction.atomic():
                    ...
                    inform_notification(
                        me, feedback.person, f"暂不公开您的反馈[{feedback.title}]。", feedback, anonymous=False)
                    inform_notification(
                        me, feedback.org, f"暂不公开您处理的反馈[{feedback.title}]。", feedback, anonymous=False)
            # 其他人没有隐藏反馈权限
            else:
    ...
    bar_display = utils.get_sidebar_and_navbar(
        request.user, navbar_name="反馈信息")
    title = feedback.title
    comments = showComment(feedback, anonymous_users=[
                           feedback.person.person_id])
...
def feedbackWelcome(request: HttpRequest):
    ...
    draft_feedback = my_all_feedback.filter(
...
def modifyFeedback(request: HttpRequest):
    ...
    all_org_types = [
        otype.otype_name for otype in OrganizationType.objects.all()]
    all_org_list = []
