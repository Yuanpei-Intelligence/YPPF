        ...
        if userinfo:
            ...
            if arg_origin is not None:
                ...
                try:
                    ...
                    name = userinfo.sname
                    return redirect(
                        arg_origin + f"?Sid={username}&Secret={en_pw}&name={name}"
                    )
...
def miniLogin(request):
    try:
        ...
        if userinfo:
            ...
            return JsonResponse({"Sname": user_account.sname, "Succeed": 1}, status=200)
        else:
...
def org_spec(request, *args, **kwargs):
    ...
    try:
        pos = position.objects.filter(
            Q(from_organization=org) | Q(job="部长") | Q(job="老板")
        )
...
def search(request):
    ...
    stu_list = student.objects.filter(
        Q(sno__icontains=query) | Q(sname__icontains=query)
    )
