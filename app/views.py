...
def couRecordShow(request):
    ERROR = [
    u'错误：表格内容为空或格式错误!',
    u'错误：表格单元格内容错误!',
    u'错误：未查询到部分学生或课程!',
    u'未选择要上传的文件！',
    u'目前尚未开放修改服务，请不要进行任何尝试！'
    ]
    html_display = { 
        'state' : 0 ,#0:ok 1:warn 2:danger
        'message' : ''
    }    
    #----身份检查----
    ...
    if user_type == "Person":
        html_display['state'] = 1
        html_display['message'] = '学生账号不能访问此界面！'
        return redirect(
                    "/welcome/"
                    + "?warn_code=1&warn_message={warn_message}".format(
                        warn_message=html_display["message"]
                    )
                )    

    year = local_dict["semester_data"]["year"]
    ...
    course = Course.objects.filter(organization = me)[0]
    positions_pos = {position.person:position.pos for position in positions}
    #positions: 储存（小组成员：职位）的一个字典

        # ---- 收集本学期已完成活动 ----
    activities=Activity.objects.activated().filter(
        organization_id=me, year = year, semester = semester).filter(
        status=Activity.Status.END)
        # ---- 统计签到情况 -----
    all_participants = Participant.objects.filter(
        activity_id__in = activities ,
        person_id__in = list(positions_pos.keys()),
    ...
    for key in list(positions_pos.keys()): #若一次也没参加活动，补为0
        if key.person_id not in list(record_list.keys()):
            record_list[key.person_id] = 0 
    for key in list(record_list.keys()): #record_list.keys()即为所有成员的列表
        found_person = NaturalPerson.objects.filter(person_id__username = key)[0]
        record_list[found_person] = record_list.pop(key) #将字典的key从学号变成<NaturePerson>
        # 下面两个属性用于传入前端
        found_person.avatar_path = utils.get_user_ava(record_list[found_person], "Person")
        found_person.pos =me.otype.get_name(positions_pos[found_person])
    ...
    edit_able = local_dict["cour_record_editable"]
    ...
    if edit_able:
        #-----新建空学时表-----
        for person in list(positions_pos.keys()): 
            ...
            if not record_search_person.exists():
                record_create = CourseRecord.objects.create(
                    person = person,
                    course = course,
                    attend_times = record_list[person],
                    total_hours = 0,
                    year = year,
                    semester = Semester.get(course_info['semester']),
                )
    ...
    for record in record_search:
        ...
        person.pos = me.otype.get_name(positions_pos[person])
    ...
    if request.method == "POST":
        if not edit_able:
            html_display["state"] = 2
            html_display["message"] = ERROR[4]
            bar_display = utils.get_sidebar_and_navbar(request.user, "课程学时")
            return render(request, "course_record.html", locals())
            ...
            for data in datas:
                ...
                person_search = NaturalPerson.objects.filter(name = data[0])
                for person_ in person_search:
                    if person in list(positions_pos.keys()): 
                        person_edit = person_
                        break
                # --- 修改学时表 ---
                ...
                record_edited[0].attend_times = data[3]
                record_edited[0].total_hours = data[2]
                record_edited[0].save()
