...
class position(models.Model):
    ...
    from_organization = models.ForeignKey(
        organization, related_name="org_from", on_delete=models.CASCADE
    )  # ,default=organization.objects.get(organization_name='暂未加入学工组织'))
    ...
    def __str__(self):
        return (
            "机构："
            + str(self.from_organization.organization_name)
            + "; 部门："
            + str(self.from_organization.department)
        )
