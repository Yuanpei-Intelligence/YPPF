...
class Position(models.Model):
    ...
    person = models.ForeignKey(
        NaturalPerson,
        related_name="position",  # 这个字段留着还是有用的
        to_field="person_id",
    ...
    class Status(models.TextChoices):  # 申请加入组织、成员换届
        JOIN = "加入组织"
        WITHDRAW = "退出组织"
        TRANSFER = "交接"
        CANCEL = "已取消"
    ...
    status = models.CharField("职位状态", choices=Status.choices, max_length=32)
