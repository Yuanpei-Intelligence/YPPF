...
def course_activity_base_check(request):
    ...
    # signscheme = int(request.POST["signscheme"]) # FIXME: doesn't exit in course activity
    # if signscheme:
    ...
    # 时间
    act_start = datetime.strptime(request.POST["actstart"], "%Y-%m-%d %H:%M")  # 活动报名时间（？应该是活动开始时间）
    act_end = datetime.strptime(request.POST["actend"], "%Y-%m-%d %H:%M")  # 活动报名结束时间（？应该是活动结束时间）
