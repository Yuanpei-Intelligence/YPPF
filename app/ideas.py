...
def create_reimbursement(request):
    ...
    2. GET
        ...
        4. 返回`render(request, create_reimbursement.html, locals())`
    3. POST
        1. 获取提交的报销信息（不含图片）
            - 有问题时最好重新根据报销信息组织页面，然后返回render
            - 无误时创建Reimbursement对象
        2. 获取提交的图片信息
            1. 获取图片列表`imgs = request.FILES.getlist(表单图片名)`
...
def comment_reimbursement(request):
    ...
    3. POST
        1. 查看是否通过
            - 如果是对应老师，更新状态，结束，提示成功并返回当前界面
                - 如果完全通过，根据要求需要先判断能否扣除元气值，并进行扣除
            - 如果不是，报错返回到当前页面
        2. 获取评论信息
            1. 如果是对应组织且修改了金额
                1. 更新数据库报销金额
